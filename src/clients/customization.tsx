...
export default function Customization() {
    ...
    const stores = useAppSelector((state) => state.GetStoresReducer.GetStoresState.GetStoresListData);
    ...
    const populate = (data: WidgetCustomizationDataResponse | null) => {
        ...
        setGreetingMessage(data.greeting_message?.trim() ? data.greeting_message : DEFAULT_GREETING);
    ...
    const updateQuickLink = (index: number, patch: Partial<QuickLinkItem>) => {
        setQuickLinks((prev) => prev.map((link, i) => (i === index ? { ...link, ...patch } : link)));
    };
        ...
        () =>
            quickLinks
                .map((link, index) => ({
                    ...
                    label: link.label ?? "",
                    url: link.url ?? "",
                    priority: Number.isFinite(link.priority) && link.priority > 0 ? link.priority : index + 1,
    ...
    const previewQuickLinks = useMemo(
        () => normalizedQuickLinks.filter((link) => link.active).sort((a, b) => a.priority - b.priority),
    ...
    const handleLogoUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!file.type.startsWith("image/")) {
            toast.error("Invalid file", { description: "Please upload a valid image (PNG, JPG, SVG, WEBP)." });
            event.target.value = "";
            return;
        }
    ...
    const handleSaveAll = async () => {
        ...
        if (normalizedQuickLinks.some((link) => link.url !== "" && !isValidUrl(link.url))) {
            toast.error("Invalid links", {
                description: "Please fix any invalid URLs in Quick Links before saving.",
            });
            return;
        }
        ...
        const quickLinksPayload: WidgetQuickLink[] = normalizedQuickLinks.map((link) => ({
        ...
        try {
            let result;
            if (!widgetExists) {
                result = await dispatch(CreateWidgetCustomization({ storeId, payload }));
                ...
                result = await dispatch(UpdateWidgetCustomizationWithImage({ storeId, payload, logoFile }));
            } else {
                result = await dispatch(UpdateWidgetCustomization({ storeId, payload }));
            }
