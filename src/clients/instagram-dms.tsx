...
import { IconBrandInstagram, IconMessage2 } from "@tabler/icons-react";
...
export default function IgDms() {
  ...
  const { FetchInstagramPagesData, FetchInstagramPagesIsLoading } =
    useAppSelector(
      (state) => state.GetSocialAIReducer.FetchInstagramPagesState,
    );
  const { FetchInstagramDmsData, FetchInstagramDmsIsLoading } = useAppSelector(
    (state) => state.GetSocialAIReducer.FetchInstagramDmsState,
  );
  ...
  // Load the connected Instagram pages for the active store.
  useEffect(() => {
    if (!storeCode) return;
    dispatch(fetchInstagramPages(storeCode));
  }, [dispatch, storeCode]);
  ...
  const selectedAccountStillExists =
    ...
    FetchInstagramPagesData.some((page) => page.id === selectedAccount);
  if (FetchInstagramPagesData.length > 0 && !selectedAccountStillExists) {
    setSelectedAccount(FetchInstagramPagesData[0].id);
  }
  const activeAccountId = selectedAccountStillExists
    ? selectedAccount
    : (FetchInstagramPagesData[0]?.id ?? null);

  // Load DMs for the selected page.
  useEffect(() => {
    if (!activeAccountId || !storeCode) return;
    dispatch(fetchInstagramDms({ accountId: activeAccountId, storeCode }));
  }, [dispatch, activeAccountId, storeCode]);

  const conversations = useMemo(
    () => groupDmsByContact(FetchInstagramDmsData),
    [FetchInstagramDmsData],
  );
        ...
        <Card className="flex min-h-0 flex-col overflow-hidden h-full">
          <div className="p-4 border-b border-border/50">
            <SocialAccountSelector
              ...
              platform="instagram"
              accounts={FetchInstagramPagesData}
              selectedId={activeAccountId}
              onSelect={setSelectedAccount}
            />
          </div>
          <div className="flex-1 min-h-0 overflow-y-auto p-3 space-y-1.5">
            {FetchInstagramPagesIsLoading || FetchInstagramDmsIsLoading ? (
              <div className="flex h-full items-center justify-center">
                <Spinner />
              </div>
            ) : !FetchInstagramPagesData.length ? (
              <div className="flex h-full items-center justify-center rounded-lg border border-dashed border-border/60 p-6 text-center text-sm text-muted-foreground">
                No connected Instagram accounts for this store.
              </div>
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <div className="relative shrink-0">
                        ...
                        <div className="absolute -bottom-1 -right-1 w-4 h-4 rounded-full bg-gradient-to-tr from-yellow-400 via-pink-500 to-purple-500 text-white flex items-center justify-center border border-background">
                          <IconBrandInstagram
                            className="w-3 h-3"
                            stroke={2.5}
                          />
