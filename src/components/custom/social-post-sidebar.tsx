...
export function SocialPostSidebar({ post }: SocialPostSidebarProps) {
  ...
  const chartData = post.moderationSummary
    ? [
        {
          name: "Positive",
          value: post.moderationSummary.positive,
          color: COLORS.positive,
        },
        {
          name: "Neutral",
          value: post.moderationSummary.neutral,
          color: COLORS.neutral,
        },
        {
          name: "Negative",
          value: post.moderationSummary.negative,
          color: COLORS.negative,
        },
        {
          name: "Spam",
          value: post.moderationSummary.spam,
          color: COLORS.spam,
        },
      ]
    : [];
    ...
    <div className="flex flex-col gap-4 p-4 h-full overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
      ...
      <Card className="border shadow-sm rounded-xl">
        ...
        <CardContent className="p-4 space-y-4">
          <div className="flex flex-col gap-3">
            {stats.map((stat, i) => (
              ...
              >
                ...
                <span
                  className={`font-medium ${stat.valueClass || "text-foreground"}`}
                >
                  {stat.value}
                </span>
              </div>
          ...
          <Button
            variant="outline"
            className="w-full text-xs h-8 gap-2 mt-2 border-border/70 shadow-sm rounded-lg font-medium"
          >
        ...
        <CardContent className="p-4 pt-3 flex flex-wrap gap-2">
          {localLabels.map((label, i) => (
            <Badge
              key={i}
              variant="secondary"
              className="font-medium text-[10px] bg-primary/[0.08] text-primary border-0 hover:bg-primary/20 px-2 py-0.5 rounded-md"
            >
              {label}
            </Badge>
          ))}
          <Popover>
            <PopoverTrigger asChild>
              <Badge
                variant="outline"
                className="font-medium text-[10px] border-dashed border-border text-muted-foreground gap-1 cursor-pointer hover:bg-muted px-2 py-0.5 rounded-md"
              >
