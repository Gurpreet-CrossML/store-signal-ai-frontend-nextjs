...
import { SocialPost, SocialAccount, MOCK_COMMENTS, SocialReply } from "@/lib/mock-social-data";
import { CustomerAvatar } from "@/components/custom/customer-avatar";
import { IconHeart, IconMessageCircle2, IconShare3, IconDots, IconChevronLeft, IconChevronRight, IconUserPlus } from "@tabler/icons-react";
...
export function SocialPostThreadView({ post, account }: SocialPostThreadViewProps) {
      ...
      <div className="flex-1 overflow-y-auto min-h-0 relative pb-10 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        ...
        <div className="p-5 flex items-center justify-between border-b sticky top-0 bg-background/95 backdrop-blur z-10">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className="font-semibold text-sm leading-tight">{account.name}</span>
          ...
          <div className="flex items-center gap-4 text-muted-foreground">
            <div className="flex flex-col items-end text-xs">
              ...
              <span>Posted {post.date.split(" at")[0]} • {account.platform === "instagram" ? "Instagram" : "Facebook"}</span>
            </div>
        ...
        <div className="p-6 pb-4">
          ...
          <div className="relative w-full rounded-xl overflow-hidden bg-muted flex items-center justify-center max-h-[400px]">
            <img src={post.image} alt={post.title} className="object-contain w-full h-full max-h-[400px]" />
            
            {/* Carousel Arrows */}
            <div className="absolute inset-x-4 top-1/2 -translate-y-1/2 flex justify-between">
              <Button variant="secondary" size="icon" className="h-8 w-8 rounded-full shadow-sm bg-background/90 hover:bg-background border">
          ...
          <div className="flex items-center gap-6 py-4">
            ...
            <div className="flex items-center gap-1.5 text-muted-foreground text-sm cursor-pointer hover:text-foreground transition-colors">
              ...
              <span className="font-semibold text-foreground">{post.commentsCount}</span>
            </div>
            <div className="flex items-center gap-1.5 text-muted-foreground text-sm cursor-pointer hover:text-foreground transition-colors">
          ...
          <div className="text-sm pb-6 border-b">
            <p className="leading-relaxed">
              Our Summer Collection is finally here! Light, breezy and made for sunny days. 
              <br/><br/>
              <span className="text-blue-600 font-medium cursor-pointer">#Summer #Fashion #WomenWear</span>
            </p>
          </div>
        </div>
        ...
        <div className="px-6 pb-6">
          <div className="flex items-center justify-between mb-5">
            <h3 className="font-semibold text-[13px] text-muted-foreground">Comments ({post.commentsCount})</h3>
            <span className="text-[11px] text-muted-foreground font-medium cursor-pointer flex items-center gap-1">Sort by: Newest <IconChevronRight className="w-3 h-3 rotate-90" /></span>
          </div>
