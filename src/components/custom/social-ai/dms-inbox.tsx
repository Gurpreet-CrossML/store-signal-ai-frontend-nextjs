...
import { AccountContext, AccountIdentity, CHANNELS, ChannelContext, SocialChannel } from "./channel-context";
...
function DmMessageBubble({
    ...
    return (
        <div className={`flex ${isOutgoing ? "justify-end" : "justify-start"}`}>
            <div className={`flex max-w-[70%] flex-col gap-1 ${isOutgoing ? "items-end" : "items-start"}`}>
                {msg.reply_to && (
                ...
                <p
                    ...
                    title={msg.external_created_at ? formatPostedAt(msg.external_created_at) : ""}
                >
                    {msg.external_created_at ? formatRelativeTime(msg.external_created_at) : ""}
                </p>
            </div>
        </div>
    );
                    ...
                    <Card className="flex min-h-0 flex-col overflow-hidden h-[88vh]!">
                        {!activeConversation ? (
                            <div className="flex h-full items-center justify-center text-sm text-muted-foreground">
                                {loading ? <Spinner /> : "Select a conversation to view messages."}
                            </div>
                        ) : (
                            <>
