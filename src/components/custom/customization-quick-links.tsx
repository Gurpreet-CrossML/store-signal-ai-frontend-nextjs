    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>
                    Links shown inside the chatbot. Order follows priority (1–9); inactive links are hidden.
                </CardDescription>
            </CardHeader>
            <CardContent>
                <div className="flex flex-col gap-3 border border-border p-4">
                    {quickLinks.map((link, index) => (
                        ...
                        >
                            ...
                            <div className="flex flex-col gap-1">
                                ...
                                <div className="flex h-8 items-center border border-input px-2.5">
                                    <input
                                        type="checkbox"
                                        checked={link.active}
                                        onChange={(event) => onUpdate(index, { active: event.target.checked })}
                            ...
                            <div className="flex flex-col gap-1">
                                <Label className="text-xs text-muted-foreground">Priority (1–9)</Label>
                                <Input
                                    type="number"
                                    min={1}
                                    max={9}
                                    ...
                                    onChange={(event) =>
                                        onUpdate(index, {
                                            priority: event.target.value === "" ? NaN : parseInt(event.target.value, 10),
                                        })
                                    }
                                />
                            </div>
                    ...
                    {quickLinks.length === 0 && (
                        <div className="flex flex-col items-center gap-1 border border-dashed border-border px-4 py-8 text-center">
                            <p className="text-xs text-muted-foreground">No quick links configured yet.</p>
                            <p className="text-xs text-muted-foreground">Click “Add link” to create one.</p>
                        </div>
                    )}
