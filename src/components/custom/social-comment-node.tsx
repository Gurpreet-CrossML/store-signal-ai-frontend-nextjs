    ...
    <div className="flex flex-col w-full">
      <div className="flex items-center gap-2 mt-1 text-muted-foreground relative w-full">
        ...
        <button
          ...
          className={cn(
            "hover:text-foreground transition-colors ml-2",
        ...
        {isAgent && (
          <IconMessageCircle2 className="h-3.5 w-3.5 text-primary ml-auto" />
        )}
      </div>
        ...
        <div className="flex items-center gap-2 mt-3 w-full animate-in fade-in slide-in-from-top-1">
          ...
          <div className="flex-1 border border-border/60 rounded-2xl px-3 py-1.5 flex items-center bg-background shadow-sm focus-within:border-primary/40 focus-within:ring-1 focus-within:ring-primary/20 transition-all">
            ...
            <button
              ...
              disabled={!canSend}
              className={cn(
                "font-semibold text-xs ml-2 transition-opacity",
                canSend
            ...
            <div key={reply.id} className="flex gap-3 relative -left-4 w-full">
              <div className="bg-background shrink-0">
                {reply.role === "agent" ? (
                  ...
                  <CustomerAvatar
                    name={reply.author}
                    src={reply.avatar || comment.avatar}
                    size="h-8 w-8"
                  />
                )}
              </div>
              ...
              <div className="flex flex-col flex-1 gap-0.5 min-w-0">
                <div className="flex items-center gap-2">
                  <span
                    className={`font-semibold text-[13px] ${reply.role === "agent" ? "text-primary" : ""}`}
                  >
                    {reply.author}
                  </span>
                </div>
                ...
                <div
                  className={`text-sm py-1.5 px-3 rounded-xl mt-1 w-fit ${reply.role === "agent" ? "bg-primary/[0.08] text-foreground/90 border border-primary/10" : "bg-muted/50"}`}
                >
