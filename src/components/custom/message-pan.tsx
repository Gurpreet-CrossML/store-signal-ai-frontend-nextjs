                                            ...
                                            {message.json_content.products.map((product: ProductData, idx: number) => {
                                                return (
                                                    <a key={idx} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-2.5 border border-border bg-background hover:bg-muted/50 transition-colors no-underline w-[240px] flex-shrink-0">
                                                        {product.image
                                                            ? <img src={product.image} alt={product.name} className="h-12 w-12 object-contain flex-shrink-0 bg-muted" />
                                                            : <div className="h-12 w-12 rounded bg-muted flex-shrink-0 flex items-center justify-center text-muted-foreground text-[10px]">No img</div>
                                                        }
                                                        ...
                                                        {product.price && <span className="text-xs font-semibold text-primary flex-shrink-0">{product.price}</span>}
                                                    </a>
                                                )
                                            })}
                                        </div>
