...
import {
  SocialCommentNode,
  renderWithMentions,
} from "@/components/custom/social-comment-node";
...
export function SocialCommentThreadView({
  comment,
  post,
  platform = "facebook",
  onClose,
}: SocialCommentThreadViewProps) {
  ...
  const [prevCommentId, setPrevCommentId] = useState<string | null | undefined>(
    comment?.id,
  );
  ...
  const handleAddTag = () => {
    if (newTagText.trim()) {
      setLocalTags((prev) => [
        ...prev,
        { label: newTagText.trim(), type: "neutral" },
      ]);
      setNewTagText("");
    }
  };
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button
                  variant="outline"
                  size="sm"
                  className="h-7 rounded-md px-2 text-muted-foreground border-dashed"
                >
                ...
                <div className="flex items-center justify-between p-2 border-t border-border/40 bg-muted/20">
                  <div className="flex items-center gap-1 text-muted-foreground">
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent
                        align="start"
                        className="flex flex-wrap gap-1 p-2 w-[160px]"
                      >
                        {["👍", "❤️", "😂", "😮", "😢", "🙏", "🔥", "✨"].map(
                          (emoji) => (
                            <button
                    ...
                    <Button
                      onClick={() =>
                        document.getElementById("file-upload-comment")?.click()
                      }
                      variant="ghost"
                      size="icon"
                      className="h-7 w-7 rounded-md hover:bg-muted hover:text-foreground"
