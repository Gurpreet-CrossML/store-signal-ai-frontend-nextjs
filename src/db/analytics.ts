...
import { and, eq, gt, gte, lte, inArray, notInArray, isNotNull, sql } from "drizzle-orm";
...
function localDateRange(column: ReturnType<typeof sql>, start: string, end: string) {
...
export async function get_conversion_analytics(params: {
    ...
    const threadFilters = [localDateRange(sql`${chatThread.createdAt}`, startDate, endDate)];
    ...
    const distinctThreadsWith = async (eventType: string, restrictTo?: string[]) => {
        const conds = [
    ...
    const othersCount = catRows.slice(4).reduce((s, r) => s + Number(r.threadCount), 0);
    ...
    const categoryDistribution: Array<{
        ...
        percentage: viewedCount ? round2((Number(item.threadCount) / viewedCount) * 100) : 0,
    }));
    ...
    const topProducts = prodRows.map((item) => ({
        ...
        threads: Number(item.threadCount),
        percentage: cartCount ? round2((Number(item.threadCount) / cartCount) * 100) : 0,
    }));
...
export async function get_feedback_insights(params: {
    ...
    const [
        ...
        db.select({ total: sql<number>`count(*)::int` })
            .from(chatThread)
            .where(threadWhere),
        db.select({ totalFb: sql<number>`count(*)::int` })
            .from(chatbotFeedback)
            .where(fbWhere),
        db.select({ rating: chatbotFeedback.rating, c: sql<number>`count(*)::int` })
            .from(chatbotFeedback)
            .where(fbWhere)
            .groupBy(chatbotFeedback.rating),
        db.select({ fcr: sql<number>`count(*)::int` })
        ...
        db.selectDistinct({ threadId: supportTicket.threadId })
            .from(supportTicket)
    ...
    const avgRating = totalFeedbacks > 0 ? round2(totalRatingScore / totalFeedbacks) : 0.0;
    ...
    const avgHandleTimeValue = durationsLen ? round2(totalSeconds / durationsLen / 60) : 0.0;
...
export async function get_user_matrix(params: {
    ...
    const filters = [localDateRange(sql`${chatThread.createdAt}`, startDate, endDate)];
    if (params.store_code) {
        filters.push(
            sql`${chatThread.storeId} IN (SELECT id FROM store WHERE code = ${params.store_code})`,
        );
...
export async function get_query_category_insights(params: {
    ...
    const analyses = await db
        ...
        .from(sentimentAnalysis)
        .where(sql`${sentimentAnalysis.threadId} IN (SELECT id FROM chat_thread WHERE ${threadWhere})`);

    const stats: Record<string, { answered: number; unanswered: number }> = {};
    for (const a of analyses) {
        const label = INTENT_TO_CATEGORY_LABEL[a.topIntent] ?? "General Conversation";
        if (!(label in stats)) stats[label] = { answered: 0, unanswered: 0 };
        const resolved = Array.isArray(a.resolvedQueriesList) ? a.resolvedQueriesList : [];
    ...
    const totalUnanswered = categoryBreakdown.reduce((s, c) => s + c.unanswered, 0);
...
export async function get_engagements(params: {
    ...
    const [
        ...
        db.select({ msgCount: sql<number>`count(${chatHistory.id})::int` })
            .from(chatHistory)
            .where(sql`${chatHistory.threadId} IN ${threadSubquery}`)
            .groupBy(chatHistory.threadId),
        // avg bot response time: per-thread avg over assistant replies
        db.select({ avgRt: sql<number | null>`avg(${chatHistory.responseTime})` })
            .from(chatThread)
            .leftJoin(
                chatHistory,
                and(
        ...
        db.select({ ret: sql<number>`count(*)::int` })
            .from(chatThread)
            .where(sql`(${chatThread.createdAt} AT TIME ZONE ${APP_TZ_SQL})::date < ${startDate}::date`),
        ...
        db.select({
            ...
            .from(chatThread)
            .where(threadWhere)
            .groupBy(sql`EXTRACT(HOUR FROM (${chatThread.createdAt} AT TIME ZONE ${APP_TZ_SQL}))`)
            .orderBy(sql`count(${chatThread.id}) desc`)
            .limit(8),
        // sessions resolved
        db.select({ resolved: sql<number>`count(*)::int` })
            .from(chatThread)
            .where(and(threadWhere, eq(chatThread.isActive, true))),
        // sessions abandoned
        db.select({ abandoned: sql<number>`count(*)::int` })
            .from(chatThread)
        ...
        db.select({
            ...
            .from(chatHistory)
            .where(sql`${chatHistory.threadId} IN ${threadSubquery}`)
            .groupBy(sql`EXTRACT(HOUR FROM (${chatHistory.createdAt} AT TIME ZONE ${APP_TZ_SQL}))`),
        // after-hours queries (role=user, ExtractHour < 9 or >= 18, current tz)
        db.select({ afterHours: sql<number>`count(*)::int` })
            .from(chatHistory)
            .where(
                and(
        ...
        db.select({
            ...
            .from(supportTicket)
            .where(localDateRange(sql`${supportTicket.createdAt}`, startDate, endDate))
            .groupBy(sql`EXTRACT(HOUR FROM (${supportTicket.createdAt} AT TIME ZONE ${APP_TZ_SQL}))`),
        ...
        db.select({
            ...
            .from(chatHistory)
            .leftJoin(sentimentAnalysis, eq(sentimentAnalysis.threadId, chatHistory.threadId))
            .where(
                and(
                    sql`${chatHistory.threadId} IN ${threadSubquery}`,
                    eq(chatHistory.role, "user"),
    ...
    for (let startHour = 0; startHour < 24; startHour++) {
        const windowHours = Array.from({ length: WINDOW_SIZE }, (_, i) => (startHour + i) % 24);
...
export async function get_operational_efficiency(params: {
    ...
    if (escalatedThreadIds.length > 0) {
        botResolvedConds.push(notInArray(sentimentAnalysis.threadId, escalatedThreadIds));
    }
    ...
    const resolutionDistribution = [
        { type: "Bot Resolved", count: botResolvedCount, percentage: botResolutionPercentage },
        { type: "Escalated", count: escalatedCount, percentage: escalationPercentage },
    ];
    ...
    const periodRows = await db
        ...
        .from(chatThread)
        .where(threadWhere)
        .groupBy(sql`date_trunc(${truncUnitSql}, ${chatThread.createdAt} AT TIME ZONE ${APP_TZ_SQL})`)
        .orderBy(sql`date_trunc(${truncUnitSql}, ${chatThread.createdAt} AT TIME ZONE ${APP_TZ_SQL})`);
    ...
    for (const row of periodRows) {
        ...
        const [{ esc }] = await db
            .select({ esc: sql<number>`count(distinct ${supportTicket.threadId})::int` })
            .from(supportTicket)
        ...
        resolutionTrend.push({
            period: periodLabel,
            bot_resolved: { count: periodBotResolved, percentage: botResolvedPercentage },
...
function parseAndValidateDaterange(
    ...
    if (!fromStr && !toStr) {
        ...
        start = fromStr ? new Date(fromStr) : new Date(now.getTime() - 7 * 86400 * 1000);
        if (toStr) {
            end = new Date(toStr);
...
export async function get_chat_history_aggregated(params: {
    ...
    const { start, end } = parseAndValidateDaterange(params.from, params.to, granularity);
...
function parseLegacyDaterange(raw: string): { start: Date; end: Date } {
    ...
    const isoMatch = trimmed.match(/^(\d{4}-\d{2}-\d{2})[^\d](\d{4}-\d{2}-\d{2})$/);
    const slashMatch = trimmed.match(/^(\d{1,2}\/\d{1,2}\/\d{4})-(\d{1,2}\/\d{1,2}\/\d{4})$/);
    ...
    const toIst = (s: string, endOfDay: boolean) => {
        ...
        if (slashDate) {
            ...
            return new Date(`${yyyy}-${mm}-${dd}T${endOfDay ? "23:59:59.999" : "00:00:00"}+05:30`);
        }
        return new Date(s);
    };
