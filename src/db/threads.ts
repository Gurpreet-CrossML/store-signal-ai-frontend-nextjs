...
import { and, asc, count, desc, eq, ilike, isNull, isNotNull, max, or, sql, SQL } from "drizzle-orm";
...
export async function list_threads(
    ...
    if (filters.is_active) {
        conditions.push(eq(chatThread.isActive, filters.is_active.toLowerCase() === "true"));
    }
    ...
    const startDate = filters.from
        ? filters.from
        : toDateString(new Date(new Date(`${endDate}T00:00:00Z`).getTime() - 30 * 24 * 60 * 60 * 1000));
    ...
    const results: ThreadListItem[] = rows.map((row) => {
        const hasCustomer = row.customer_email !== null || row.customer_first_name !== null;
...
export async function get_thread_details(
    ...
    const messageRows = await db
        ...
        .from(chatHistory)
        .where(eq(chatHistory.threadId, thread_id))
        .orderBy(limit != null ? desc(chatHistory.createdAt) : asc(chatHistory.createdAt))
    ...
    const hasCustomer = t.customer_email !== null || t.customer_first_name !== null;
 ...
 */
export async function get_cart_data(thread_id: string): Promise<CartData | null> {
    const rows = await db
        .select({
...
export async function get_thread_tags(
    ...
    const tags = rows.length && rows[0].tags != null ? (rows[0].tags as string[]) : [];
    return { tags };
}
