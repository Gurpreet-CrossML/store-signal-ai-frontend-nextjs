...
import { djangoContentType, authPermission, authGroup, authGroupPermissions, authUser, authUserGroups, authUserUserPermissions, djangoAdminLog, store, chatbotWidgetCustomization, chatbotWidgetCustomizationQuickActions, quickAction, quickLink, storeCredentials, storeFaqs, chatCustomer, chatAddress, chatThread, chatHistory, chatbotFeedback, chatBotevent, aiInsights, sentimentAnalysis, sessionResolutionVerdict, userMetadata, fraudFlag, scrapeLinkslinks, knowledgeStorelibrarydocument, supportTicket } from "./schema";
...
export const chatbotWidgetCustomizationRelations = relations(chatbotWidgetCustomization, ({one, many}) => ({
	store: one(store, {
		fields: [chatbotWidgetCustomization.storeId],
		references: [store.id]
	}),
	chatbotWidgetCustomizationQuickActions: many(chatbotWidgetCustomizationQuickActions),
...
export const chatbotWidgetCustomizationQuickActionsRelations = relations(chatbotWidgetCustomizationQuickActions, ({one}) => ({
	chatbotWidgetCustomization: one(chatbotWidgetCustomization, {
		fields: [chatbotWidgetCustomizationQuickActions.chatbotwidgetcustomizationId],
...
export const storeCredentialsRelations = relations(storeCredentials, ({one}) => ({
	store: one(store, {
...
export const sessionResolutionVerdictRelations = relations(sessionResolutionVerdict, ({one}) => ({
	store: one(store, {
...
export const scrapeLinkslinksRelations = relations(scrapeLinkslinks, ({one}) => ({
	store: one(store, {
...
export const knowledgeStorelibrarydocumentRelations = relations(knowledgeStorelibrarydocument, ({one}) => ({
	store: one(store, {
