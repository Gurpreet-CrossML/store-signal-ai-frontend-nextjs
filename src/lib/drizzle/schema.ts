import { pgTable, bigint, varchar, timestamp, unique, integer, index, foreignKey, boolean, check, text, smallint, jsonb, uuid, doublePrecision, numeric, inet } from "drizzle-orm/pg-core"
...
export const storeCredentials = pgTable("store_credentials", {
	...
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	storeId: bigint("store_id", { mode: "number" }).notNull(),
	supportTicketApiKey: text("support_ticket_api_key"),
	supportTicketApiUrl: varchar("support_ticket_api_url", { length: 200 }),
	supportTicketPlatform: varchar("support_ticket_platform", { length: 20 }),
	supportTicketUsername: text("support_ticket_username"),
}, (table) => [
	foreignKey({
			columns: [table.storeId],
			foreignColumns: [store.id],
...
export const supportTicket = pgTable("support_ticket", {
	...
	platformCreatedAt: timestamp("platform_created_at", { withTimezone: true, mode: 'string' }),
	platformUpdatedAt: timestamp("platform_updated_at", { withTimezone: true, mode: 'string' }),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).notNull(),
	updatedAt: timestamp("updated_at", { withTimezone: true, mode: 'string' }).notNull(),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	customerId: bigint("customer_id", { mode: "number" }),
	...
	index("support_tic_platfor_507db3_idx").using("btree", table.platform.asc().nullsLast().op("text_ops")),
	...
	index("support_tic_thread__d6c413_idx").using("btree", table.threadId.asc().nullsLast().op("uuid_ops"), table.createdAt.asc().nullsLast().op("uuid_ops")),
	index("support_tic_ticket__1d6a0f_idx").using("btree", table.ticketId.asc().nullsLast().op("int8_ops")),
