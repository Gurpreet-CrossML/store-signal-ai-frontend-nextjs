...
export const slackChannel = pgTable(
  "slack_channel",
  {
    ...
    takeoverStartedBy: varchar("takeover_started_by", { length: 64 }),
    createdAt: timestamp("created_at", {
      withTimezone: true,
      mode: "string",
    }).notNull(),
  ...
  (table) => [
    index("slack_channel_channel_id_d4068f29_like").using(
      "btree",
      table.channelId.asc().nullsLast().op("varchar_pattern_ops"),
    ),
    foreignKey({
      columns: [table.threadId],
      foreignColumns: [chatThread.id],
      name: "slack_channel_thread_id_d9f094bb_fk_chat_thread_id",
    }),
    unique("slack_channel_channel_id_key").on(table.channelId),
    unique("slack_channel_thread_id_key").on(table.threadId),
  ],
);
