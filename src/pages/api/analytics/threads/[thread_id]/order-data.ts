import { get_order_data } from "@/db/threads";
...
async function handler(req: NextApiRequest, res: NextApiResponse<APIResponse>) {
  ...
  const data = await get_order_data(thread_id as string);

  if (!data) {
    return res
      .status(404)
      .json(
        createAPIResponse(false, "No order data found for this thread", []),
      );
  }

  return res
    .status(200)
    .json(createAPIResponse(true, "Order data retrieved successfully", data));
}
