import { list_facebook_dms } from "@/db/social";
...
/**
 * GET /social/facebook/dms?account_id=&store_code= — DMs for one Facebook
...
async function handler(req: NextApiRequest, res: NextApiResponse<APIResponse>) {
  ...
  const dms = await list_facebook_dms(
    accountId,
  ...
  return res
    .status(200)
    .json(createAPIResponse(true, "Facebook DMs retrieved successfully.", dms));
}
