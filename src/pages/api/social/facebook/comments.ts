import { list_facebook_comments } from "@/db/social";
...
/**
 * GET /social/facebook/comments?post_id=&store_code= — comments on one
 * Facebook post (`social_message` where message_type = 'comment').
 */
...
async function handler(req: NextApiRequest, res: NextApiResponse<APIResponse>) {
  ...
  const comments = await list_facebook_comments(
    postId,
  ...
  return res
    ...
    .json(
      createAPIResponse(
        true,
        "Facebook comments retrieved successfully.",
        comments,
      ),
    );
}
