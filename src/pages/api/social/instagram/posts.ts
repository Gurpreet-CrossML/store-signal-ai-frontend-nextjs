import { list_instagram_posts } from "@/db/social";
...
/**
 * GET /social/instagram/posts?account_id=&store_code= — posts for one Instagram
...
async function handler(req: NextApiRequest, res: NextApiResponse<APIResponse>) {
  ...
  const posts = await list_instagram_posts(
    accountId,
  ...
  return res
    .status(200)
    .json(
      createAPIResponse(true, "Instagram posts retrieved successfully.", posts),
    );
}
